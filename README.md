...
Требуется исследовать случай с доминирующим собственным значением равным единицы.


- Если решение сходиться за менее чем 2 итерации, то мы подозреваем что 
  результат может быть не верным, к примеру сходиться ко собственному вектору
  2 формы. Поэтому добавляем возмущение и запускаем итерации заново. Тем
  самым мы ограничиваем, чтобы быстро не сходилось.
- При малых значениях |𝜦2|/|𝜦1|, решение быстро сходиться и не требует 
  перезапуска.
- Если матрица не имеет доминирующей собственного значения, то прерывается
  решение с ошибкой.
